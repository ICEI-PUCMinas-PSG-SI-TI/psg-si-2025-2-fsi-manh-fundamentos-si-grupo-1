...
async function gerar() {
  const res = await apiFaker.criarUnidadesMedida(recurse.value, quant.value)
  if (res.ok) {
    notificacoes.addNotification('Unidades de medida criadas com sucesso!', { time: 3000 })
  }
}
  ...
  <div class="card bg-base-100 border-base-300 border-2">
    <div class="card-body">
      <div class="flex flex-col gap-2 p-2">
        ...
        <hr />
        <p><strong>Deletar unidades de medida por ID</strong></p>
        <p>
          Observação: Isso também ira deletar outras informações associadas as {entidades}
          deletadas.
        </p>
        ...
        <ButtonComponent class="btn-primary btn-disabled" @click="show">
          Mostrar unidades de medida no console
        </ButtonComponent>
      </div>
    </div>
  </div>
