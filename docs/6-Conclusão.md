...
Apesar dos avanços alcançados, algumas limitações ainda precisam ser reconhecidas. A solução atual não inclui integração com sistemas externos, como módulos de vendas ou financeiro, o que poderia fortalecer o fluxo de informações da empresa. Alguns processos, como previsão de demanda e análise avançada de indicadores, também não foram implementados nesta versão inicial. Além disso, como todo sistema em adoção, será necessário um período de acompanhamento, treinamento e ajustes conforme o uso real pelos colaboradores.

Para trabalhos futuros, recomenda-se a expansão do sistema com funcionalidades avançadas, como análises preditivas para reposição automática, integração com leitura por código de barras ou RFID e dashboards gerenciais mais robustos. Estudos de usabilidade e avaliações de impacto organizacional também podem contribuir para a evolução contínua da ferramenta.

De forma geral, o projeto demonstrou resultados expressivos, entregando uma solução digital que aumenta a eficiência operacional da empresa parceira ao mesmo tempo em que proporcionou aos estudantes uma experiência real e enriquecedora de desenvolvimento ágil e centrado no usuário.
